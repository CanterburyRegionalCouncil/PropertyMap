    ...
    var Widget = declare("esri.dijit.Geocoder", [_EventedWidget, _TemplatedMixin], {
        ...
        _performTask: function (def, e) {
            ...
            if (e.search) {
                ...
                // Make a call to the predictve search service
                esriRequest({
                    url: this.serviceURL,
                    content: {
                        f: "pjson",
        ...
        _handleSuggestionInfo: function (data, def, e) {
            this._receivedResults(data.searchResults, def);
        },
        _handleSuggestionError: function (err) {
            console.log('geocode widget suggestion request error: ', err);
        },
        ...
        _inputKeyUp: function (e) {
            if (e) {
                ...
                if (e.ctrlKey || e.shiftKey || e.metaKey || e.altKey || e.keyCode === keys.copyKey || e.keyCode === keys.ALT || e.keyCode === keys.CTRL || e.keyCode === keys.META || e.keyCode === keys.SHIFT || e.keyCode === keys.UP_ARROW || e.keyCode === keys.DOWN_ARROW || e.keyCode === keys.LEFT_ARROW || e.keyCode === keys.RIGHT_ARROW) {
                    ...
                    console.log("_inputKeyUp: autoComplete");
                    this._autoComplete();
                } else {
                    // hide menus
                    this._hideMenus();
        ...
        _hydrateResult: function (e) {
            ...
            if (e.hasOwnProperty('label')) {
                var result = {};
                result.text = e.label;
                result.magicKey = null; //JSON.stringify(e.value);
                result.isCollection = false;
                result.suggestion = e.value;
